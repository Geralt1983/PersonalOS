...
export function BrainDump({ entries, tags, onAdd, onDelete, onUpdate, onArchive, onCreateTag }: BrainDumpProps) {
  ...
  return (
    <Card className="steel-card border-nebula-blue/20 flex flex-col">
      ...
      <CardContent className="space-y-3 flex-1 flex flex-col">
        {showFilters && (
          <div className="space-y-2 p-2 rounded-lg steel-surface border border-border/30">
        ...
        <div className="space-y-2 flex-1 overflow-y-auto pr-1 max-h-[300px]">
          {filteredEntries.length === 0 ? (
            <div className="text-center py-6 text-sm text-muted-foreground">
              {entries.length === 0 ? "Your mind is clear. For now." : "No matching thoughts found."}
            </div>
          ) : (
            filteredEntries.map((entry) => (
              <div
                ...
                className="p-2.5 rounded-lg steel-surface border border-border/30 hover:border-nebula-blue/30 transition-colors group"
              >
